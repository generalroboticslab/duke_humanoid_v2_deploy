# Twin of control/ipc/publisher.py (the CANONICAL copy; control/common until 2026-08-22). Byte-identical to it
# below this header since 2026-08-22, when strict_map_key=False was ported to
# both msgpack.unpackb calls here (before that this copy silently dropped any
# int-keyed packet). Fix the canonical copy first, then mirror it here.
# Importers: imu/py_imu.py only (humanoid_mission_recorder.py moved to ipc.publisher
# on 2026-08-22). See ../README.md.
import socket
import urllib.parse
import threading
import time
...
class NNGSubscriber:
    ...
    Args:
        connect_url:      pynng URL to connect to, e.g. ``"tcp://<host>:9873"``.
        encoding:         ``"msgpack"`` (default) or ``"json"``.
        recv_timeout_ms:  Recv timeout in milliseconds (default 200).

    Example::

        sub = NNGSubscriber("tcp://<host>:9873")
        sub.start()
        # … later …
    ...
    def __init__(self, connect_url: str, encoding: str = "msgpack",
        ...
        self._decode = (
            # strict_map_key=False: allow non-string dict keys (e.g. int camera
            # ports in MonitorDetectionPacket.frame_id_by_port). Without it msgpack
            # raises on every such packet and the subscriber silently drops it.
    ...
    def _loop(self):
        ...
        # Retry dial until connected or stopped — publisher may not be up yet.
        while self._running:
            try:
...
class DataPublisher:
    ...
    When ``thread=True`` (default), ``publish()`` only updates a latest-value
    slot and signals a daemon sender thread (latest-value-wins semantics).
    This keeps the caller's hot loop non-blocking without FIFO backlog.
    ...
    def __init__(
        ...
        **socket_kwargs,
        ...
        # Threading: publish() stores only the latest sample and signals sender thread.
        # This keeps caller non-blocking while avoiding FIFO backlog latency.
        self._threaded = thread
        if thread:
            self._latest_data: dict[str, Any] | None = None
            self._has_data = False
    ...
    def _enqueue(self, data: dict):
        """Hot-path for threaded mode: overwrite stale data and signal sender."""
        with self._data_lock:
            self._latest_data = data
            self._has_data = True
        self._wake_event.set()
    ...
    def _publisher_loop(self):
        """Background thread: send latest data only (latest-value-wins)."""
...
class DataReceiver:
    ...
    DECODINGS = {
        ...
        'msgpack': lambda data: msgpack.unpackb(data, raw=False, strict_map_key=False),
